...
GComponent <- R6Class("GComponent",
  inherit = BasicToolkitInterface,
  public = list(
    toplevel = NULL,            # toplevel instance
    constructor = NULL,         # for write_constructor
    args = NULL,                # for write_constructor
    prefix= "o",                # really just "o"
    id = character(),           # the actual id, e.g gWidget_ID2
    transport_signal = "",      # if given, the signal to initiate transport
    value = NULL,                   # default property of the object
    public_methods = character(),   # exposed to rpc call
    ##
    .e = NULL,                  # for stashing things (tag, tag<-)
    change_signal = "",         # what signal is default change signal
    connected_signals = list(),
    coerce_with = NULL,         # used in the derived GWidget class
    ..visible = TRUE,
    ..enabled = TRUE,
    ..editable = TRUE,
    ..index = NULL, 
    ...
    initialize = function(container = NULL, parent = NULL,  toplevel = NULL, ...) {
      "Initialize widget. @param toplevel required or found"
      if (is(toplevel, "GWidgetsTopLevel")) {
        self$toplevel <- toplevel
      } else {
        if(is.null(container) & !is.null(parent))
          self$toplevel <- parent$get_toplevel()
        if(!is.null(container))
          self$toplevel <- container$get_toplevel()
      }

      ## This involves a hack for finding the
      ## toplevel when none is specified, which
      ## should only happen with the initial
      ## gwindow() call.
      ## Insert a toplevel thingy

      if(is.null(self$toplevel)) {
      ...
      self$args$extend(list(id=self$id))
    ...
    add_args = function(..., overwrite=TRUE) {
      ...
      sapply(list(...), self$args$extend, overwrite=overwrite)
    },
    ...
    get_id = function() {
      ...
      sprintf("%s%s", self$prefix, self$id)
    },
    ...
    add_js_queue = function(cmd) {
      ...
      self$toplevel$js_queue_push(cmd)
    },
    flush_js_queue = function() {
      "Flush commands in JavaScript queue"
      self$toplevel$js_queue_flush()
    },
...
Thised to the javascript queue withing the transport function call
E.g.is.getText()}"
    ...
    add_handler = function(signal, handler, action=NULL, decorator, ...) {
      ...
      ID <- NULL
      if (self$is_handler(handler)) {
        if (!missing(decorator))
          handler <- decorator(handler)
        o <- observer(self, handler, action) # in gWidgets2 but not now
        ID <- self$add_observer(o, signal)
      }
      self$connect_to_toolkit_signal(signal, ...)
      invisible(ID)
    },
    ...
    connect_to_toolkit_signal = function(
      ...
      ## only connect once
      if(is.null(self$connected_signals[[signal, exact=TRUE]]))
       self$add_R_callback(signal, ...)
      self$connected_signals[[signal]] <- TRUE
    },
    ...
    add_R_callback = function(signal, ...) {
      "Add a callback into for the Ext signal. Return callback id as a list."
      ...
      cmd <- sprintf("%s.on('%s', function(%s) {%s; callRhandler('%s', '%s', param)}, null, {delay:10, buffer:100, single:false});",
                     ...
                     signal,
                     self$cb_args(signal),
                     self$param_defn(signal),
                     self$get_callback_object(),
                     signal
                     )
      self$add_js_queue(cmd)
    ...
    before_handler = function(signal, params) {
      ...
      if(signal == self$change_signal) {
        self$process_transport(params)
      }
    },
    ## delayed call
    add_async_javascript_callback = function(url, callback,  data=list(), 
...
"
      out <- whisker.render(tpl, list(url = url,
                                      ...
                                      data = toJSObject(
                                        merge.list(list(session_id = String("session_id")),
                                                   data)),
                                      callback=callback))
                               add_js_queue(out)
    },
    ...
    add_handler_double_click=function(handler, action=NULL, ...) {
      self$add_handler("dblclick", handler, action, ...)
    },
    ##
    ## rpc
    ##
    ...
    add_public_method=function(x) {
      "Add a method name to the public methods, so that jRpc can call back into R."
      self$public_methods <- c(self$public_methods, x)
    },
    ##
    ## Drag and Drop
    ##
    ...
    setup = function(container, handler, action=NULL, ext.args=NULL, ...) {
      ...
      ##  by one with this.
      if(length(self$transport_signal) == 1 &&
         self$transport_signal == self$change_signal)
        self$write_change_transport()
      else
    ...
    set_cookie = function(key, value) {
      "Set a cookie"
      self$toplevel$cookies[[key]] <- value
    },
    get_cookies = function() {
      "Return list of cookies"
      self$toplevel$the_request$cookies()
    },
    ##
    ...
    set_index=function(value, ...) {
      ...
      self$..index <- value
      if(!identical(old_index, self$..index))
        self$invoke_change_handler()
    },
    get_visible = function() self$..visible,
    set_visible = function(value) {
      self$..visible <- as.logical(value)
      self$call_Ext("setVisible", as.logical(value))
    },
    ...
    set_enabled = function(value) {
      ...
      self$..enabled <- value
      if(value)
        self$call_Ext("enable")
      else
        self$call_Ext("disable")
    },
    ...
    set_attr = function(key, value) {
      ...
      attr(self$.e, key) <- value
    },
     set_height = function(px) {
       "Set height in pixels"
       self$call_Ext("setHeight", px)
     },
     set_width = function(px) {
       "Set width in pixels"
       self$call_Ext("setWidth", px)
     },
