...
#'
#' Useful if installing from a package which doesn't bundle. The extjs files
#' are quite large (nearly 70Mb download), so rather than bundle in an R
#' package, one can download as needed.
#' @return Nothing. Installs files if successful
#' @export
download_extjs <- function() {
    to <- paste(system.file("base", "javascript", package="gWidgetsWWW2"), 
                "ext-4-2.1", sep=.Platform$file.sep)
...
#'
#' @param x a list that will map to an object literal. E.g., list(a=1, b="a") 
#'   goes to {a:1, b:'a'}
