...
\item the \pkg{canvas} device along with the \code{gcanvas}
  ...
  \code{canvas} device. A handful of reference class methods are
  available, as illustrated in the example.
...
their container, unlike the canvas object. There are browser
variations with SVG graphics.
