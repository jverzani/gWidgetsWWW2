...
share the evaluation environments. To do this, the \pkg{filehash}
package is used to store the, to a directory specified to
\code{getOption("gWidgetsWWW2:db_name")}. This usage is significantly
slower to both create the page and to handle any callbacks into the R process.
