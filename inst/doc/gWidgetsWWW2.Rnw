...
programming, and \R's internal web server is used for serving pages,
through the \pkg{Rook} package of Jeffrey Horner.

The primary contribution of the package is a) a means to create
JavaScript from \R\/ code (from the \pkg{gWidgets} API) and b) a means
to easily incorporate this into a web page.  These web pages are not
centered around displaying content, but rather for creating
interactive controls. However, if desired, the package can be used to
integrate in with an existing web page, such as those provided through
\pkg{brew} templates.
...
common use then is to use nested containers with different packing directions:
<<>>=
w <- gwindow("simple GUI with one button", visible=FALSE)
...
\item The widgets to select from a vector of values:
  \code{gcheckbox}, \code{gcheckboxgroup}, \code{gradio}, \code{gcomobox};
...
\item Images can be viewd through \code{gimage} -- the image is a url
...
The basic dialogs are implemented except \code{gbasicdialog}). These
...
\pkg{nginx}, say, to serve pages to the public. The \pkg{nginx} web
server is basically a proxy, routing requests to the underlying
\pkg{Rook} process. 
\begin{quotation}
...
so a configuration  in \code{nginx.conf} like:
